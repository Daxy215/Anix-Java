...
final class Complier {
	...
	public void compile() throws Exception {
		 ...
		 * DiagnosticCollector<JavaFileObject>(); StandardJavaFileManager fileManager =
		 * compiler.getStandardFileManager(diagnostics, Locale.getDefault(), null);
		 * List<JavaFileObject> javaObjects = null;
		 * 
		 * if(ProjectSettings.isEditor) { javaObjects =
		 * scanRecursivelyForJavaObjects(sourceDir, fileManager); } else { javaObjects =
		 * scanRecursivelyForJavaObjects(classesDir, fileManager); }
		 * 
		 * if (javaObjects == null || javaObjects.size() == 0) { return; }
		 * 
		 ...
		 * diagnostics, compilationOptions, null, javaObjects);
		 * 
		 * if (!compilerTask.call()) { for(Diagnostic<?> diagnostic :
		 * diagnostics.getDiagnostics()) {
		 * //System.err.format("[Error] on line %d in %s", diagnostic.getLineNumber(),
		 * diagnostic); Console.LogErr("[Error] on line " + diagnostic.getLineNumber() +
		 * " in " + diagnostic + "\n");
		 * 
		 * String path =
		 * diagnostic.getSource().toString().split("\\[")[1].split("\\]")[0]; String[]
		 * splitPath = path.split("\\\\"); String name = splitPath[splitPath.length -
		 * 1];
		 * 
		 * failers.add(name); }
		 * 
		 ...
		 * 
		 * Editor.importedClasses.add(new AudioPlayer()); Editor.importedClasses.add(new
		 * BoxCollider2D()); Editor.importedClasses.add(new Button());
		 * Editor.importedClasses.add(new Camera()); Editor.importedClasses.add(new
		 * LightSource()); Editor.importedClasses.add(new MeshRenderer());
		 * Editor.importedClasses.add(new Physics2D()); Editor.importedClasses.add(new
		 * SpriteRenderer());
		 * 
		 * classLoader = URLClassLoader.newInstance(new
		 * URL[]{classesDir.toURI().toURL()});
		 * Thread.currentThread().setContextClassLoader(classLoader); Field scl =
		 * ClassLoader.class.getDeclaredField("scl"); // Get system class loader
		 * scl.setAccessible(true); // Set accessible scl.set(null, classLoader); //
		 * Update it to your class loader
		 * 
		 * for(int i = 0; i < files.size(); i++) { try { //TODO:
		 * java.lang.ClassFormatError: Truncated class file????? Class<?> clazz =
		 * Class.forName(files.get(i).getName().split(".java")[0].trim(), true,
		 * classLoader); //addSoftwareLibrary(files.get(i));
		 * 
		 * if(clazz != null) { Object obj = clazz.getConstructor().newInstance();
		 * 
		 * try { if(obj instanceof Behaviour) {
		 * Editor.importedClasses.add((Behaviour)obj); }
		 * 
		 * System.out.println("Successfully loaded class with the name of: " +
		 * files.get(i).getName().split(".java")[0]); } catch(Exception e) {
		 * System.err.println("[ERROR] " + clazz.getSimpleName() + " : " +
		 * e.getMessage()); } } } catch(Exception | NoClassDefFoundError e) {
		 * System.err.println("[ERORR] [TSH] Couldn't load a class with the name of: " +
		 * files.get(i).getName().split(".java")[0] + " because of " + e.getMessage());
		 * e.printStackTrace(); } }
		 */
	}
	...
	private List<JavaFileObject> scanRecursivelyForJavaObjects(File dir, StandardJavaFileManager fileManager) {
		...
		for (File file : files) {
			if (file.isDirectory()) {
				...
				System.err.println("Loaded class with the name of: " + file.getName());
				this.files.add(file);
			}
		}
	...
	private JavaFileObject readJavaObject(File file, StandardJavaFileManager fileManager) {
		...
		throw new RuntimeException(
				"[ERROR] [TSH] #110 Could not load " + file.getAbsolutePath() + " java file object!");
	}
